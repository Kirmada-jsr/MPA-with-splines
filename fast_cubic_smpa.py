...
@njit
def cubic_bspline_gradient(x_control, y_control, x_data, y_data, ti, class_1_pseudo, degree=3):
    n = len(x_control)
    m = len(x_data)
    grad = np.zeros(n)
    knot_len = degree + (n - 1) + degree
    ...
    for i in range(m):
        ...
        factor = (1 / n_err) * ti[i] * class_1_pseudo
        for k in range(n):
            grad[k] += factor * cubic_bspline_basis(t, k, degree, knots)
    return grad

class FastCubicSMPA:
    def __init__(self, n_control_points=6, max_iter=50, lambda_reg=0.0001, smoothing_factor=0.0001, verbose=False):
    ...
    def _to_numpy(self, data):
        ...
        if isinstance(data, np.ndarray):
            return data
        elif hasattr(data, 'to_numpy'):  # Pandas DataFrame or Series
            return data.to_numpy()
        elif hasattr(data, 'numpy'):  # PyTorch tensor
            return data.detach().cpu().numpy()
        else:  # Try generic conversion (lists, etc.)
            try:
    ...
    def fit(self, X, y):
        ...
        for j in range(n_features):
            ...
            def loss(y_ctrl):
                ...
                hinge_loss = np.mean(np.maximum(0, z))
                ...
                if self.verbose:
                    print(f"Feature {j}, Loss: {total_loss:.6f}, Hinge: {hinge_loss:.6f}, Smoothness: {smoothness_penalty:.6f}")
                return total_loss
...
if __name__ == "__main__":
    ...
    model = FastCubicSMPA(n_control_points=6, max_iter=50, lambda_reg=0.001, verbose=True)
    model.fit(X, y)
